...
public class CamFieldGeometry implements Serializable
{
	...
	public CamFieldGeometry(int lineWidth, int fieldLength, int fieldWidth, int boundaryWidth, int refereeWidth,
			...
			int defenseStretch, int freeKickFromDefenseDist, int penaltySpotFromFieldLineDist, int penaltyLineFromSpotDist)
	{
	...
	{
		lineWidth = fs.getLineWidth();
		fieldLength = fs.getFieldLength();
		fieldWidth = fs.getFieldWidth();
		boundaryWidth = fs.getBoundaryWidth();
		refereeWidth = fs.getRefereeWidth();
		goalWidth = fs.getGoalWidth();
		goalDepth = fs.getGoalDepth();
		goalWallWidth = fs.getGoalWallWidth();
		centerCircleRadius = fs.getCenterCircleRadius();
		defenseRadius = fs.getDefenseRadius();
		defenseStretch = fs.getDefenseStretch();
		freeKickFromDefenseDist = fs.getFreeKickFromDefenseDist();
		penaltySpotFromFieldLineDist = fs.getPenaltySpotFromFieldLineDist();
		penaltyLineFromSpotDist = fs.getPenaltyLineFromSpotDist();
	}
