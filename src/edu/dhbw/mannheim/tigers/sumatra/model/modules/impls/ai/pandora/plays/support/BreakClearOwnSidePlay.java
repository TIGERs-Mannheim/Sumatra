...
import edu.dhbw.mannheim.tigers.sumatra.model.data.modules.ai.fieldraster.EnhancedFieldAnalyser.ESearchAlgorithm;
import edu.dhbw.mannheim.tigers.sumatra.model.data.modules.ai.fieldraster.EnhancedFieldAnalyser.ESituation;
 ...
 * 
 */
public class BreakClearOwnSidePlay extends ASupportPlay
{
	...
	private IVector2							penaltyStartingPointForSearch	= AIConfig.getGeometry().getPenaltyMarkOur()
																									.addNew(new Vector2(+100, 0));
	 ...
	 */
	public BreakClearOwnSidePlay(AIInfoFrame aiFrame, int numAssignedRoles)
	{
	...
	private IVector2 calcTarget(AIInfoFrame aiFrame, ARole role)
	{
		...
		penaltyStartingPointForSearch = AIConfig.getGeometry().getPenaltyMarkOur().addNew(new Vector2(+100, 0));
		if (target == null)
		{
			do
			{
				target = aiFrame.tacticalInfo.getEnhancedFieldAnalyser().getBestPositionInNearOfPoint(ESituation.FREE,
						ESearchAlgorithm.HILLCLIMBING, penaltyStartingPointForSearch);
	...
	private IVector2 searchTarget(AIInfoFrame aiFrame, ARole role)
	{
		...
		if (analyser != null)
		{
			...
			{
				target = getTarget(aiFrame, role, listQuadrantsEmpty, 3, 4);
			}
			
			if (botMap.get(role) == EWAI.OTHER)
			{
				target = getTarget(aiFrame, role, listQuadrantsEmpty, 4, 3);
			}
		}
		return target;
	}
	...
	private boolean checkDestination(AIInfoFrame aiFrame, IVector2 target, ARole role)
	{
		...
		if (penCirc.isPointInShape(aiFrame.worldFrame.ball.getPos()))
		{
			return false;
		}
