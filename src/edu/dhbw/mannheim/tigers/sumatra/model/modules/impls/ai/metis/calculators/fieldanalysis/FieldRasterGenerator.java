...
public final class FieldRasterGenerator
{
	...
	@Configurable(comment = "")
	private static int						analysingFactor	= 16;
	...
	{
		if ((fieldNumber >= 0) && (fieldNumber < (numberOfColumns * numberOfRows)))
		{
			return true;
		}
		return false;
	...
	{
		if ((fieldNumber >= 0) && (fieldNumber < (numberOfColumns * numberOfRows * analysingFactor * analysingFactor)))
		{
			return true;
		}
		return false;
	...
	{
		final int numberOfFields = (numberOfColumns * numberOfRows * analysingFactor * analysingFactor) - 1;
	...
	{
		final int numberOfFields = (numberOfColumns * numberOfRows) - 1;
		if (verifyAnalysisField(numberOfFields))
		{
			for (int i = 0; i <= numberOfFields; i++)
			{
	 ...
	 */
	public IVector2 getRandomPointInPosRec(int fieldNumber)
	{
	 ...
	 */
	public IVector2 getRandomConditionPoint(int fieldNumber, SimpleWorldFrame worldFrame, BotID botId,
	 ...
	 * @return the nearest of 20 random points where the target is visible
	 */
	public IVector2 getRandomConditionPoint(AIRectangle rectangle, SimpleWorldFrame worldFrame, BotID botId,
			AVisibleCon condition)
	{
		IVector2 point = rectangle.getRandomPointInShape();
		final IVector2 botPos = new Vector2(worldFrame.getBot(botId).getPos());
		
		float distanceMin = GeoMath.distancePP(botPos, point);
		
		for (int i = 0; i < RANDOM_TRIES; i++)
		{
			final IVector2 newPoint = rectangle.getRandomPointInShape();
	 ...
	 * 
	 */
	public IVector2 getRandomPointWithConditions(AIRectangle rectangle, SimpleWorldFrame worldFrame, BotID botId,
			List<AVisibleCon> conditions)
	{
		
		final IVector2 safetyPoint = rectangle.getRandomPointInShape();
		IVector2 bestPoint = new Vector2(GeoMath.INIT_VECTOR);
	 ...
	 */
	public static int getAnalysingFactor()
	{
		return analysingFactor;
	}
	 ...
	 * @return
	 */
	public static int getNumberOfPositioningFields()
	{
	 ...
	 * @return
	 */
	public static int getNumberOfAnalysingFields()
	{
		return getNumberOfPositioningFields() * analysingFactor * analysingFactor;
	}
}
