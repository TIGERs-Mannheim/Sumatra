 ...
 * 
 */
public class TurnTestSkill extends AMoveSkill
{
	 ...
	 * @param angle
	 */
	public TurnTestSkill(float size, float angle)
	{
		super(ESkillName.TURN_TEST);
		this.size = size;
		this.angle = angle;
	}
	...
	protected List<ACommand> doCalcEntryActions(TrackedTigerBot bot, List<ACommand> cmds)
	{
		...
		{
			nodes.add(GeoMath.stepAlongCircle(bot.getPos(), bot.getPos().addNew(new Vector2(size, 0)),
					(((angle) / (PATH_POINTS - 1)) * i)));
		}
		
		createSpline(bot, nodes, bot.getPos().addNew(new Vector2(size, 0)));
		return cmds;
	}
	// --------------------------------------------------------------------------
	// --- getter/setter --------------------------------------------------------
