 ...
 * 
 */
public class AIRectangle extends Rectangle implements Comparable<AIRectangle>
{
	// --------------------------------------------------------------------------
	...
	{
		final int row = ((rectangleID - 1) / (FieldRasterGenerator.getNumberOfColumns() * FieldRasterGenerator
				.getAnalysingFactor())) + 1;
		
		if ((row == 1) || (row == (FieldRasterGenerator.getNumberOfRows() * FieldRasterGenerator.getAnalysingFactor())))
		{
			return true;
		}
		
		final int rowMod = rectangleID
				% (FieldRasterGenerator.getNumberOfColumns() * FieldRasterGenerator.getAnalysingFactor());
	...
	public float getTotalNeighboursValue()
	{
		...
		for (AIRectangle rect : neighbours)
		{
			if (rect != null)
			{
				sum += rect.getValue();
