...
public class BaseStationWifiStatsPanel extends JPanel
{
	...
	// --------------------------------------------------------------------------
	private final Map<BotID, WifiStatsPanel>	wifiPanels			= new TreeMap<BotID, WifiStatsPanel>(
																							BotID.getComparator());
	...
	public void setStats(final BaseStationWifiStats stats)
	{
		...
		EventQueue.invokeLater(() -> {
			updateRate.setText("" + stats.getUpdateRate() + "Hz");
		});
		
		for (BotID id : new ArrayList<BotID>(wifiPanels.keySet()))
		{
	...
	private static class WifiStatsPanel extends JPanel
	{
		...
		private JProgressBar[]		nrfStats				= new JProgressBar[4];
		private JProgressBar[]		queueStats			= new JProgressBar[2];
		private JLabel[]				queueTraffic		= new JLabel[2];
		private JProgressBar			linkQuality;
		...
		public WifiStatsPanel()
		{
			...
			add(id, "span 1 2");
			add(linkQuality, "span 1 2, height 40px");
		...
		public void setStats(final BotStats stats, final int rate)
		{
			EventQueue.invokeLater(() -> {
				...
				if (stats.getBotId().isBot())
				{
					...
					id.setText("" + stats.getBotId().getNumber());
				}
					else
					{
						setVisible(false);
