...
public class BallPossessionDetection implements IEventTypeDetection<BallPossessionEventType>
{
	...
	public void nextFrameForDetection(TypeDetectionFrame frame)
	{
		...
		} else if (previousSimpleWorldFrame != null)
		{
			...
			Set<BotID> currentlyTouchingBots = botLastTouchedBallCalculator.currentlyTouchingBots(currentSimpleWorldFrame)
					.stream()
			...
			} else
			{
				...
				} else
				{
					...
					ballPossession = new BallPossessionEventType(ballPossession.getPossessionState(),
							ballPossession.getRobot().orElse(null));
				}
			}
	...
	private void checkIfBallContentionStateStillActive(final SimpleWorldFrame currentSimpleWorldFrame,
			final List<IDrawableShape> shapes)
	{
		if (ballContentionState)
		{
			...
			long amountBotsOutOfThreshold = botsInvolvedInBallContention.stream()
					.map(currentSimpleWorldFrame::getBot)
					.filter(Objects::nonNull)
					.filter(bot -> bot.getPos().distanceTo(ballPos) - Geometry.getBotRadius()
							> leaveBallContentionStateThresholdDistance)
					.count();
