...
public class DefenseMath
{
	...
	static
	{
		ConfigRegistration.registerClass("math", DefenseMath.class);
	}
	 ...
	 */
	private static ILineSegment getThreatDefendingLine(
			...
			final double maxGoOutDistance)
	{
		...
		IVector2 end = Geometry.getPenaltyAreaOur().withMargin(marginToPenArea)
				.nearestPointOutside(base, protectionPos);
			...
			final double maxGoOutDistance)
	{
		...
		base = Geometry.getPenaltyAreaOur().withMargin(marginToPenArea).nearestPointOutside(base, threatPos);
	 ...
	 * @return
	 */
	public static ILineSegment getThreatDefendingLineForManToManMarker(final ILineSegment threatLine,
			final double marginToThreat)
	{
		return getThreatDefendingLine(threatLine, marginToThreat,
				DefenseConstants.getMinGoOutDistance(),
				DefenseConstants.getMaxGoOutDistance());
	}
	 ...
	 */
	public static List<ReceiveData> calcReceiveRatingsFor(final IBallTrajectory ballTrajectory,
			final Collection<ITrackedBot> bots, final double maxCheckDistance)
	{
		...
		for (ITrackedBot bot : bots)
		{
			...
			ratings.add(new ReceiveData(bot, ballCurve, ballTrajectory.getPosByTime(0)));
		}
		return ratings;
	}
	
	/** Data holder for PassReceiverCalc */
