...
public class BestDirectShotBallPossessingBot
{
	...
	@Configurable(comment = "Angle which indicates a shot in this angle is a goal in almost every case", defValue = "0.35")
	private static double probablyAGoalAngle = 0.35;

	@Configurable(comment = "opponent shot speed")
	...
	static
	{
		ConfigRegistration.registerClass("offensive", BestDirectShotBallPossessingBot.class);
	}
	...
	{
		return getBestShot(goal, shotPos, obstacleBots, 0.0);
	}
	...
	{
		return getBestShot(goal, shotPos, obstacleBots, new ArrayList<>(), 20, timeUntilShot);
	}
	...
	{
		return getBestShot(goal, shotPos, obstacleBots, shapes, 20, 0.0);
	}
			...
			List<IDrawableShape> shapes, int nIterations, double timeUntilShot)
	{
		...
		if (!bestTarget.isPresent() && nIterations > 1)
		{
		...
		for (int i = 0; i < nIterations; ++i)
		{
			...
			for (ITrackedBot bot : obstacleBots)
			{
				...
				if (vaguePosition.getCenter().x() > shotPos.x() + 2 * Geometry.getBallRadius()
						&& (curArc.isPointInShape(vaguePosition.getCenter(), vaguePosition.getRadius())
						|| Lines.segmentFromPoints(curArc.center(),
								curArc.center().addNew(Vector2.fromAngle(curArc.getStartAngle()).scaleTo(curArc.radius())))
			...
			final List<FreeArea> freeAreas, final List<IDrawableShape> shapes)
	{
		for (FreeArea freeArea : freeAreas)
		{
			...
			IVector2 interceptionWithGoalLeft = goalLine.intersectLine(
					Lines.lineFromDirection(shotPos, Vector2.fromAngle(freeArea.getStartAngle().getAngle())))
					.orElse(Vector2.zero());
			IVector2 interceptionWithGoalRight = goalLine.intersectLine(
