...
public class RepulsivePassReceiver extends ARepulsiveBehavior
{
	...
	{
		Map<BotID, CalcViabilityInfo> botViabilityForRepulsiveBehavior = ((SupportRole) getRole())
				.getBotViabilityForRepulsiveBehavior();
		...
		getRole().getWFrame().getBots().values().stream()
				.filter(bot -> !((SupportRole) getRole()).getCurrentSupportBots().contains(bot) && bot.getTeamColor() == getRole().getAiFrame().getTeamColor())
				.forEach(bot -> botViabilityForRepulsiveBehavior.put(bot.getBotId(), new CalcViabilityInfo(false)));

		long deltaTime = calcDeltaTime();

		if (botViabilityForRepulsiveBehavior.get(getRole().getBotID()) == null)
		{
			botViabilityForRepulsiveBehavior.put(getRole().getBotID(), new CalcViabilityInfo(passReceiver));
		}

		// Refresh History rating factors
		botViabilityForRepulsiveBehavior.forEach((bot, info) -> info.nextCalcDone(deltaTime));

		if (passReceiver && isActive)
		{
			if ((botViabilityForRepulsiveBehavior.get(getRole().getBotID()) != null) &&
					!botViabilityForRepulsiveBehavior.get(getRole().getBotID()).isActive)
			{
				botViabilityForRepulsiveBehavior.put(getRole().getBotID(), new CalcViabilityInfo(true));
			}
		...
		else
		{
			if ((botViabilityForRepulsiveBehavior.get(getRole().getBotID()) != null) &&
					botViabilityForRepulsiveBehavior.get(getRole().getBotID()).isActive)
			{
				botViabilityForRepulsiveBehavior.put(getRole().getBotID(), new CalcViabilityInfo(false));
			}
	...
	private int getNumberPassReceiver()
	{
		...
		return Math.max(0, numberPassReceiver);
	}
	...
	{
		Map<BotID, CalcViabilityInfo> botViabilityForRepulsiveBehavior = ((SupportRole)getRole()).getBotViabilityForRepulsiveBehavior();

		double score = Geometry.getFieldLength() * 0.5d;

		score += bot.getPos().x();

		CalcViabilityInfo botViabilityHistory = botViabilityForRepulsiveBehavior.get(bot.getBotId());
			...
			final List<ITrackedBot> opponents)
	{
		...
		idx = (int) getRole().getAiFrame().getPlayStrategy().getActiveRoles(ERole.SUPPORT).stream()
				.filter(b -> getRole().getBall().getPos().distanceToSqr(b.getPos()) < distToBall).count() + 1;
	...
	private Force getOffensiveSightForce(final IVector2 currentPos)
	{
		...
		if (offensiveBot.isPresent()
				&& getRole().getAiFrame().getTacticalField().getSkirmishInformation().getStrategy()
				!= ESkirmishStrategy.NONE)
		{
			...
			final ITrackedBot affectedBot)
	{
		...
		for (ITrackedBot opponent : opponents)
		{
			...
			forces.add(new Force(referencePoint, sigmaBehindOpponentLine, magnitudeBehindOpponentLine));
		}
