...
public class MaxAngleKickRaterTest
{
	...
	{
		double prob = MaxAngleKickRater.getDirectShootScoreChance(wf.getOpponentBots().values(),
				Geometry.getGoalOur().getCenter());
		Assert.assertTrue(prob < 0.50);
	}


	@Test
	public void OpponentLongDistanceTest()
	{
		double prob = MaxAngleKickRater.getOpponentScoreChanceWithDefender(wf.getTigerBotsVisible().values(),
	...
	/**
	 * Tests points in a half circle around the opponent goal to make sure the chance is not too low
	 */
	@Test
	public void OurShortDistanceTest()
	{
		...
		for (int i = 0; i < testpoints - 1; i++)
		{
			...
			double prob = MaxAngleKickRater.getDirectShootScoreChance(wf.getOpponentBots().values(), testpos);
	...
	@Test
	public void OpponentShortDistanceTest()
	{
		double biggestProb = 0;
		int testpoints = 8; // actually tests n-2 points to avoid testing points on extended goal line
		...
		for (int i = 0; i < testpoints - 1; i++)
		{
			...
			double prob = MaxAngleKickRater.getOpponentScoreChanceWithDefender(wf.getTigerBotsVisible().values(), testpos);
			biggestProb = Math.max(biggestProb, prob);
		}
		Assert.assertTrue(biggestProb > 0.9);
	}
	...
	{
		for (ITrackedBot bot : wf.getOpponentBots().values())
		{
			...
			double prob = MaxAngleKickRater.getDirectShootScoreChance(wf.getOpponentBots().values(), pos);
	...
	@Test
	public void OpponentBehindBotsTest()
	{
		for (ITrackedBot bot : wf.getTigerBotsVisible().values())
		{
			...
			double prob = MaxAngleKickRater.getOpponentScoreChanceWithDefender(wf.getTigerBotsVisible().values(), pos);
