...
public class KickBallObstacleV2 implements IObstacle, IDrawableShape
{
	 ...
	 * @param bot
	 */
	public KickBallObstacleV2(final ITrackedBall ball, final IVector2 openDir, final ITrackedBot bot)
	{
		this.ball = ball;
	...
	private List<IObstacle> getObstacle(final double t)
	{
		...
		IVector2 center = ball.getTrajectory().getPosByTime(t);
		double ellXradius = (secRadius - freeFront) / 2;
		Ellipse frontEll = Ellipse.createTurned(center.addNew(openDir.scaleToNew(-(freeFront + ellXradius))), ellXradius,
		...
		double dist = VectorMath.distancePP(circleCenterL, circleCenterS);
		int numCircles = 10;
		for (int i = 0; i < numCircles; i++)
		{
			IVector2 c1 = LineMath.stepAlongLine(circleCenterL, circleCenterS, (dist * i) / (numCircles - 1));
			...
			shapes.add(new CircleObstacle(Circle.createCircle(c1, r)));
			shapes.add(new CircleObstacle(Circle.createCircle(c2, r)));
		}
