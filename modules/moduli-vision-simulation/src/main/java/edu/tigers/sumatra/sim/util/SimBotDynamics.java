...
public class SimBotDynamics
{
	...
	public void step(final double dt, final BotSkillOutput output)
	{
		...
		double rotate = AngleMath.difference(pos.z(), orientBefore);
		velLocal.setXY(velLocal.getXYVector().turn(-rotate));
		accLocal.setXY(accLocal.getXYVector().turn(-rotate));
	}
	
	
	private void stepW(final double dt, final BotSkillOutput output)
	{
	...
	private void stepXY(final double dt, final BotSkillOutput output)
	{
	...
	private void globalPosXY(final double dt, final BotSkillOutput output)
	{
		...
		if (output.isStrictVelocityLimit() && velLocal.getLength2() > output.getDriveLimits().getVelMax())
		{
			velLocal.setXY(velLocal.getXYVector().scaleTo(output.getDriveLimits().getVelMax()));
		}
	}
}
